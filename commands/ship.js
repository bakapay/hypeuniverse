...
let vemoji = client.guilds.get("418461344707248129").emojis.find("name", "CHV")
	...
	const rating = Math.floor((Math.random() * 120));
...
if(rating < 100) return message.channel.send("**" + message.author.username + "** :heart: **" + user.username + "** **|** **" + rating + "%** - Amazing :heart_eyes:")
if(rating > 100) return message.channel.send("**" + message.author.username + "** :heart: **" + user.username + "** **|** **100%** - PERFECT :heart_exclamation:")	
};
